...
public class ChromaColorHandle: UIView, ChromaControlStylable {
  ...
  func makeCirclePath(shapeFrame: CGRect) -> CGPath{
    ...
    internal func layoutHandleShape() {
        ...
        handleShape.path = makeCirclePath(shapeFrame: CGRect(origin: .zero, size: size)) //makeHandlePath(frame: CGRect(origin: .zero, size: size))

        //handleShape.frame = CGRect(origin: CGPoint(x: bounds.midX - (size.width / 2), y: bounds.midY - (size.height / 2)), size: size)
      handleShape.frame = CGRect(origin: CGPoint(x: bounds.midX, y: bounds.midY), size: size)
