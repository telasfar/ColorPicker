...
public class ChromaColorPicker: UIControl, ChromaControlStylable {
  ...
  @objc func handleTap(_ touches:UIGestureRecognizer ) {
    ...
    internal let colorWheelView = ColorWheelViewCustom()
    internal var colorWheelViewWidthConstraint: NSLayoutConstraint!
    
    internal func commonInit() {
        self.backgroundColor = UIColor.white
        setupColorWheelView()
      addGestureRecognizer(UIPanGestureRecognizer(target: self, action: #selector(handleTap(_:))))
    ...
    internal func positionHandle(_ handle: ChromaColorHandle, forColorLocation location: CGPoint) {
        handle.center = location
          //.applying(CGAffineTransform.identity.translatedBy(x: handle.bounds.height / 2, y: handle.bounds.height / 2))
    }
